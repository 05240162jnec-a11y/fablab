...
export default function WhatsAppButton({ message, size = 'md', className = '', phone = null, countryCode = null }) {
    ...
    useEffect(() => {
        if (phone) return; // Direct number provided (Pattern 2) - no need to fetch FabLab number
    ...
    // ✅ Pattern 2: direct phone number provided (e.g. customer contact for production team)
    if (phone !== null || countryCode !== null) {
        const cleanPhone = (phone || '').replace(/^0+/, ''); // strip leading zero if present
        ...
        if (!isValid) {
            return (
                <div
                    ...
                    className={`inline-flex items-center justify-center ${sizeClasses[size]} bg-gray-300 rounded-full shadow-sm flex-shrink-0 cursor-not-allowed ${className}`}
                >
                    <WhatsAppIcon />
                </div>
            );
        ...
        return (
            ...
            href = {`https://wa.me/${fullNumber}?text=${encodeURIComponent(message || '')}`}
    target = "_blank"
    rel = "noopener noreferrer"
    onClick = {(e) => e.stopPropagation()
}
title = "Contact customer on WhatsApp"
className = {`inline-flex items-center justify-center ${sizeClasses[size]} bg-green-500 hover:bg-green-600 rounded-full shadow-sm hover:shadow-md transition-all flex-shrink-0 ${className}`}
            >
    <WhatsAppIcon />
            </a >
        );
...
// ✅ Pattern 1: fixed FabLab number (fetched from settings)
if (!fablabNumber) return null;

return (

    href = {`https://wa.me/${fablabNumber}?text=${encodeURIComponent(message || '')}`}
target = "_blank"
rel = "noopener noreferrer"
onClick = {(e) => e.stopPropagation()}
title = "Contact FabLab on WhatsApp"
className = {`inline-flex items-center justify-center ${sizeClasses[size]} bg-green-500 hover:bg-green-600 rounded-full shadow-sm hover:shadow-md transition-all flex-shrink-0 ${className}`}
        >
    <WhatsAppIcon />
        </a >
