...
export default function ProductionTeamInventory() {
    ...
    // ✅ Fetch inventory data from API (using same admin endpoints)
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="px-6 pb-6 space-y-6">
                    ...
                    <>
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="px-8 py-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div>
                                    <h2 className="text-xl font-bold text-gray-900">Received Records</h2>
                                    <p className="text-sm text-gray-500 mt-1">Material receiving history</p>
                                </div>
                                <button
                            ...
                            {receivedRecords.length === 0 ? (
                                <div className="py-20 text-center">
                                    ...
                                    <p className="text-gray-600 font-semibold text-lg">No received records yet</p>
                                </div>
                            ) : (
                                    ...
                                    <table className="w-full">
                                        <thead>
                                            <tr className="bg-gray-50 border-b border-gray-200">
                                                <th className="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Sl.No</th>
                                                ...
                                                <th className="px-8 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Qty</th>
                                                <th className="px-8 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Rate (Nu.)</th>
                                                <th className="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Date</th>
                                                <th className="px-8 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Total (Nu.)</th>
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {receivedRecords.map((record) => (
                                                <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                                                    <td className="px-8 py-5 text-sm text-gray-600">{record.slNo}</td>
                                                    <td className="px-8 py-5 text-sm font-semibold text-gray-900">{record.name}</td>
                                                    <td className="px-8 py-5 text-sm text-gray-600">{record.desc || '-'}</td>
                                                    <td className="px-8 py-5 text-sm font-semibold text-gray-900 text-right">{record.qty}</td>
                                                    <td className="px-8 py-5 text-sm text-gray-600 text-right">Nu. {record.rate.toFixed(2)}</td>
                                                    <td className="px-8 py-5 text-sm text-gray-600">{record.date}</td>
                                                    <td className="px-8 py-5 text-sm font-bold text-gray-900 text-right">Nu. {record.total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                        ...
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mt-6">
                            <div className="px-8 py-6 border-b border-gray-100">
                                <h2 className="text-xl font-bold text-gray-900">Stock Update</h2>
                                <p className="text-sm text-gray-500 mt-1">Current inventory status (highlighted if &lt; 10)</p>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead>
                                        <tr className="bg-gray-50 border-b border-gray-200">
                                            <th className="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Sl.No</th>
                                            <th className="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Material Name</th>
                                            <th className="px-8 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Previous Stock</th>
                                            <th className="px-8 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Received</th>
                                            <th className="px-8 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">New Stock</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {stockData.map((stock, index) => (
                                            ...
                                            >
                                                <td className="px-8 py-5 text-sm text-gray-600">{stock.slNo}</td>
                                                <td className="px-8 py-5 text-sm font-semibold text-gray-900">{stock.name}</td>
                                                <td className="px-8 py-5 text-sm text-gray-600 text-right">{stock.prevStock} units</td>
                                                <td className={`px-8 py-5 text-sm font-semibold text-right ${stock.received > 0 ? 'text-green-600' : 'text-gray-600'}`}>
                                                    {stock.received > 0 ? `+${stock.received}` : stock.received} units
                                                </td>
                                                <td className="px-8 py-5 text-sm font-bold text-gray-900 text-right">{stock.newStock} units</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                ...
                {!loading && !error && activeTab === 'issued' && (
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        <div className="px-8 py-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div>
                                <h2 className="text-xl font-bold text-gray-900">Issued Records</h2>
                                <p className="text-sm text-gray-500 mt-1">Material issuance history</p>
                            </div>
                            <button
                        ...
                        {issuedRecords.length === 0 ? (
                            <div className="py-20 text-center">
                                ...
                                <p className="text-gray-600 font-semibold text-lg">No issued records yet</p>
                            </div>
                        ) : (
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead>
                                        <tr className="bg-gray-50 border-b border-gray-200">
                                            <th className="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Sl.No</th>
                                            <th className="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Item Name</th>
                                            <th className="px-8 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Qty Issued</th>
                                            <th className="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Issued To / Reason</th>
                                            <th className="px-8 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Date</th>
                                            <th className="px-8 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {issuedRecords.map((record) => (
                                            <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-8 py-5 text-sm text-gray-600">{record.reason || '-'}</td>
                                                <td className="px-8 py-5 text-sm text-gray-600">{record.date}</td>
                                                <td className="px-8 py-5 text-center">
                                                    <button
            ...
            {showAddModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl z-10 border border-gray-200">
                        <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-6">
                            <h3 className="text-xl font-bold text-gray-900">Add Received Record</h3>
                            <button onClick={toggleAddModal} className="text-gray-400 hover:text-gray-600 transition-colors p-2 hover:bg-gray-100 rounded-lg">
                        ...
                        <form onSubmit={addToTable} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="md:col-span-2 relative" ref={dropdownRef}>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm bg-white pr-12 transition-all"
                                    />
                                    <button
                                        type="button"
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-700 font-bold text-lg">
                                    Nu. {calculateTotal()}
                                </div>
                            </div>
                            ...
                            <div className="md:col-span-2 flex justify-end gap-3 mt-4 pt-6 border-t border-gray-100">
                                <button
                                    type="button"
                                    onClick={toggleAddModal}
                                    className="px-6 py-3 text-sm font-semibold text-gray-600 hover:bg-gray-100 rounded-xl transition-all"
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    className="px-8 py-3 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 transition-all shadow-lg hover:shadow-xl"
                                >
                                    Save Record
                                </button>
                            </div>
            ...
            {showIssueModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl z-10 border border-gray-200">
                        <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-6">
                            <h3 className="text-xl font-bold text-gray-900">Issue Material</h3>
                            <button onClick={toggleIssueModal} className="text-gray-400 hover:text-gray-600 transition-colors p-2 hover:bg-gray-100 rounded-lg">
                                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                            </button>
                        ...
                        <form onSubmit={issueMaterial} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {/* Reason */}
                            <div className="md:col-span-2">
                                <label className="block text-sm font-semibold text-gray-700 mb-2">Issued To / Reason</label>
                                <input
                                    type="text"
