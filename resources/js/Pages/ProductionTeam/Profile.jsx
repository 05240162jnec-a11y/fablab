...
export default function ProductionTeamProfile() {
    ...
    const [editingFields, setEditingFields] = useState({ name: false, email: false, phone: false, gender: false });
    ...
    // ── Same constraints as Register.jsx ──
    const weakPasswords = [
    ...
    // ── API calls — all use sessionStorage + /api/production-team/ ──
    const fetchProfile = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                setUser(userData);
                setEditForm({ name: userData.name || '', email: userData.email || '', phone: userData.phone || '', gender: userData.gender || '' });
    ...
    const handleSaveProfile = async (e) => {
        ...
        setEditLoading(true); setSuccessMessage(null); setError(null);
        try {
            const token = sessionStorage.getItem('auth_token');
            ...
            if (response.data.success) {
                ...
                setEditingFields({ name: false, email: false, phone: false, gender: false });
                setProfilePhoto(null); setPhoneError('');
                fetchProfile();
            }
        } catch (err) {
            console.error(err);
    ...
    const handlePasswordChange = async (e) => {
        ...
        if (pwdErrs.length > 0) { setPasswordError('Password must contain: ' + pwdErrs.join(', ')); return; }
        if (passwordForm.new_password !== passwordForm.new_password_confirmation) { setConfirmError('New passwords do not match'); return; }
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="px-6 pb-6 space-y-6">
                ...
                {error && user && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">⚠️ {error}</div>}
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            <div className="w-16 h-16 rounded-full flex items-center justify-center text-white text-xl font-bold shadow-md overflow-hidden cursor-pointer"
                                ...
                                {photoPreview ? <img src={photoPreview} alt="Profile" className="w-full h-full object-cover" /> : user?.name?.charAt(0) || 'P'}
                            </div>
                        ...
                        <div className="flex-1">
                            ...
                            <span className="inline-flex mt-1 px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700 border border-green-200">Production Team</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                        ...
                        <form onSubmit={handleSaveProfile} className="space-y-4">
                            ...
                            {/* Phone — country dropdown + digit-limited input */}
                            <div>
                                <div className="flex items-center justify-between mb-2">
                                ...
                                {editingFields.phone ? (
                                    <>
                                        <div style={{ display: 'flex', gap: '.625rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                                            <div style={{ flexShrink: 0 }}>
                                                <CountrySelect value={phoneCountry}
                                                    onChange={(code) => { setPhoneCountry(code); if (editForm.phone) setPhoneError(validatePhone(editForm.phone, code)); }}
                                                    error={!!phoneError} />
                                            </div>
                                        ...
                                        {phoneError
                                            ...
                                            : <p className="text-xs text-gray-500 mt-1">{countries.find(c => c.code === phoneCountry)?.flag} {getCountryPhoneRules(phoneCountry).hint}</p>}
                                    </>
                                ) : (
                            ...
                            <form onSubmit={handlePasswordChange} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div style={{ position: 'relative' }}>
                                        <input type={showCurrPwd ? 'text' : 'password'} value={passwordForm.current_password}
                                ...
                                <div>
                                    ...
                                    <div style={{ position: 'relative' }}>
                                        <input type={showNewPwd ? 'text' : 'password'} value={passwordForm.new_password}
                                            onChange={e => {
                                                const val = e.target.value;
                                                setPasswordForm(prev => ({ ...prev, new_password: val }));
                                                if (val) { const errs = validatePassword(val); setPasswordError(errs.length > 0 ? 'Must contain: ' + errs.join(', ') : ''); }
                                            ...
                                            className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm ${passwordError ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
                                            style={{ paddingRight: '2.75rem' }} required />
                                ...
                                <div>
                                    ...
                                    <div style={{ position: 'relative' }}>
                                        <input type={showConfirmPwd ? 'text' : 'password'} value={passwordForm.new_password_confirmation}
                                            onChange={e => {
                                                const val = e.target.value;
                                            ...
                                            className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm ${confirmError ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
                                            style={{ paddingRight: '2.75rem' }} required />
