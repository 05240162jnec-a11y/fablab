            ...
            <div className="modal-box" onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-inner">
                    ...
                    <div className="modal-info-side">
                        {product.description && (
                            <p className="modal-tag-line">{product.description}</p>
                        )}
                        ...
                        <div className="modal-price-stock">
                            ...
                            {inStock
                                ? <span className="modal-badge-stock in">{product.stock} available</span>
                                : <span className="modal-badge-stock out">Out of stock</span>
                            }
                        </div>

                        {product.size && (
                            <p className="modal-size-line">Size: {product.size}</p>
                        )}
                        ...
                        <div className="modal-actions">
                            {inStock ? (
                                <>
                                    <button className="btn-add-to-cart-modal" onClick={() => onAddToCart(product, qty)}>
                                        Add to Cart
                                    </button>
                                    <button className="btn-buy-now-modal" onClick={() => { onBuyNow(product, qty); onClose(); }}>
                                        Buy Now
                                    </button>
                                </>
                            ) : (
...
export default function Shop() {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                // Try public endpoint first; fall back to auth endpoint if needed
                let res = await fetch(`${API_BASE}/home/products`, {
                    headers: { 'Accept': 'application/json' }
                });

                if (res.status === 401 && token) {
                    res = await fetch(`${API_BASE}/user/products`, {
                        headers: {
    ...
    const handleAddToCart = useCallback((product, qty = 1) => {
        if (!isLoggedIn) {
            showToast('Please login to add to cart', 'warn');
            return;
        }
        setCart(prev => {
    ...
    const handleBuyNow = useCallback((product, qty = 1) => {
        if (!isLoggedIn) {
            showToast('Please login to place an order', 'warn');
            return;
        }
        ...
        <div style={{ fontFamily: "'DM Sans', sans-serif", background: '#f9fafb', color: '#0d1117', overflowX: 'hidden' }}>
            <style>{`
                ...
                * { box-sizing:border-box; margin:0; padding:0; }
                ...
                /* ── PRODUCT GRID: 4 columns matching image 1 ── */
                .products-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:1.5rem; }

                /* ── PRODUCT CARD — exact image-1 layout ── */
                .product-card {
                    background: #fff;
                ...
                /* image area */
                .product-img-wrap { position:relative; height:200px; overflow:hidden; background:#e8ecf0; flex-shrink:0; }
                .product-img-wrap img { width:100%; height:100%; object-fit:cover; transition:transform .55s ease; display:block; }
                .product-card:hover .product-img-wrap img { transform:scale(1.06); }

                /* price tag in top-right corner (like image 1) */
                .product-price-tag {
                ...
                .oos-label { background:var(--red); color:white; font-size:.875rem; font-weight:800; padding:.5rem 1.25rem; border-radius:9999px; letter-spacing:.02em; }
                ...
                .view-detail-pill { padding:.4rem 1.1rem; background:white; color:var(--ink); font-size:.78rem; font-weight:700; border-radius:9999px; border:none; cursor:pointer; transition:all .2s; }
                .view-detail-pill:hover { background:var(--blue); color:white; }

                /* card body */
                .product-body { padding:.9rem 1rem 1rem; display:flex; flex-direction:column; gap:.2rem; flex:1; }
                .product-name { font-size:.9rem; font-weight:700; color:var(--ink); line-height:1.4; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
                .product-size { font-size:.75rem; color:var(--muted); }

                /* bottom row: price left, Add to Cart right — same as image 1 */
                .product-bottom {
                    display:flex; align-items:center; justify-content:space-between;
                    gap:.5rem; margin-top:.6rem;
                }
                .product-price { font-family:'Playfair Display',serif; font-size:1rem; font-weight:900; color:var(--blue); white-space:nowrap; }
                .btn-add-cart {
                    padding:.45rem 1rem;
                    background:var(--blue); color:white;
                    font-size:.78rem; font-weight:700; border-radius:.55rem;
                    border:none; cursor:pointer; white-space:nowrap;
                    box-shadow:0 3px 10px rgba(26,86,219,.25);
                    transition:all .22s;
                    flex-shrink:0;
                }
                .btn-add-cart:hover { background:var(--blue-dk); transform:translateY(-1px); box-shadow:0 6px 18px rgba(26,86,219,.35); }
                .btn-add-cart-disabled { padding:.45rem 1rem; background:#e2e8f0; color:#94a3b8; font-size:.78rem; font-weight:700; border-radius:.55rem; border:none; white-space:nowrap; cursor:not-allowed; flex-shrink:0; }
                ...
                /* ── TOAST — image 2 style: dark maroon warning ── */
                .toast-wrap {
                ...
                /* ── MODAL — image 3 style ── */
                .modal-backdrop { position:fixed; inset:0; z-index:999; background:rgba(5,10,25,.6); backdrop-filter:blur(5px); display:flex; align-items:center; justify-content:center; padding:1.5rem; animation:fadeIn .2s ease; }
                @keyframes fadeIn { from{opacity:0}to{opacity:1} }
                .modal-box { background:white; border-radius:1.25rem; width:100%; max-width:820px; max-height:90vh; position:relative; overflow:hidden; box-shadow:0 32px 80px rgba(0,0,0,.3); animation:modalUp .25s ease; display:flex; flex-direction:column; }
                @keyframes modalUp { from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)} }
                .modal-close { position:absolute; top:1rem; right:1rem; z-index:10; width:34px; height:34px; border-radius:50%; background:rgba(255,255,255,.9); border:1px solid var(--border); display:flex; align-items:center; justify-content:center; cursor:pointer; color:var(--muted); transition:all .2s; }
                .modal-close:hover { background:#f1f5f9; color:var(--ink); }
                .modal-inner { display:grid; grid-template-columns:1.05fr 1fr; flex:1; overflow:auto; }

                /* gallery side */
                .modal-gallery-side { background:#f8fafc; display:flex; flex-direction:column; gap:.75rem; padding:1.5rem; }
                .modal-main-img-wrap {
                    position:relative; flex:1; min-height:260px; max-height:340px;
                    border-radius:.85rem; overflow:hidden; background:#edf0f4;
                ...
                .modal-arrow {
                    ...
                    display:flex; align-items:center; justify-content:center;
                    color:var(--ink); box-shadow:0 2px 10px rgba(0,0,0,.15);
                    transition:all .2s;
                }
                .modal-arrow:hover { background:white; box-shadow:0 4px 16px rgba(0,0,0,.2); }
                .modal-arrow-left  { left:.65rem; }
                .modal-arrow-right { right:.65rem; }
                .modal-thumbs { display:flex; gap:.5rem; flex-wrap:wrap; }
                .modal-thumb { width:58px; height:58px; border-radius:.5rem; overflow:hidden; border:2.5px solid transparent; padding:0; cursor:pointer; background:#e2e8f0; flex-shrink:0; transition:border-color .2s; }
                .modal-thumb.active { border-color:var(--blue); }
                .modal-thumb img { width:100%; height:100%; object-fit:cover; display:block; }

                /* info side */
                .modal-info-side { padding:2rem 1.75rem 2rem 1.5rem; display:flex; flex-direction:column; overflow-y:auto; }
                .modal-tag-line { font-size:.82rem; color:var(--muted); margin-bottom:.5rem; line-height:1.5; }
                .modal-prod-title { font-family:'Playfair Display',serif; font-size:1.5rem; font-weight:800; color:var(--ink); letter-spacing:-.02em; margin-bottom:.875rem; line-height:1.25; }
                .modal-price-stock { display:flex; align-items:center; gap:.75rem; margin-bottom:.5rem; flex-wrap:wrap; }
                .modal-price { font-family:'Playfair Display',serif; font-size:1.5rem; font-weight:900; color:var(--blue); }
                ...
                .qty-control { display:flex; align-items:center; gap:0; border:1.5px solid var(--border); border-radius:.55rem; overflow:hidden; }
                .qty-btn { width:34px; height:34px; background:var(--offwhite); border:none; cursor:pointer; font-size:1.1rem; font-weight:600; color:var(--ink-2); transition:background .2s; display:flex; align-items:center; justify-content:center; }
                .qty-btn:hover { background:#e2e8f0; }
                .qty-val { min-width:36px; text-align:center; font-size:.9rem; font-weight:700; color:var(--ink); padding:0 .25rem; }
                .modal-actions { display:flex; gap:.75rem; flex-wrap:wrap; margin-top:auto; padding-top:.5rem; }
                .btn-add-to-cart-modal {
                    flex:1; padding:.8rem 1rem;
                    background:white; color:var(--blue);
                    font-size:.9rem; font-weight:700; border-radius:.65rem;
                ...
                .btn-buy-now-modal:hover { background:var(--blue-dk); box-shadow:0 6px 20px rgba(26,86,219,.4); transform:translateY(-1px); }
                .btn-modal-disabled { display:block; width:100%; padding:.8rem; background:#e2e8f0; color:#94a3b8; font-size:.9rem; font-weight:700; border-radius:.65rem; border:none; text-align:center; cursor:not-allowed; }
                ...
                <div className="page-hero-inner">
                    ...
                    <div className="page-hero-stats">
                        {[
                            ...
                            [loading ? '…' : `${totalStock}`, 'Units in Stock'],
                            ['Nu.', 'Local Currency'],
                        ].map(([n, l]) => (
                            <div key={l}>
                                ...
                                <div key={i} style={{ background: '#fff', borderRadius: '1.25rem', border: '1px solid rgba(0,0,0,.08)', overflow: 'hidden' }}>
                                    ...
                                    <div style={{ padding: '.9rem 1rem 1rem', display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '.4rem' }}>
                                            ...
                                            <div className="skeleton" style={{ height: 34, width: '44%', borderRadius: '.55rem' }} />
                                        </div>
                                    </div>
                                </div>
                            ))}
                                    ...
                                    <Reveal key={product.id} delay={Math.min((idx % 4) * 0.08, 0.3)}>
                                        <div className="product-card" onClick={() => setSelected(product)}>
                                            <div className="product-img-wrap">
                                                ...
                                                {/* Price tag top-right like image 1 */}
                                                ...
                                                <div className="product-hover-overlay">
                                                    <button className="view-detail-pill"
